...
//    long long int nptoss = tosses / world_size;
    ...
    if ( world_rank == 0 ) {
        recv = ( long long int * )malloc( world_size * sizeof(long long int) );
    }
    
    MPI_Gather(&toss_in_circle, 1, MPI_LONG_LONG, recv, 1, MPI_LONG_LONG, 0, MPI_COMM_WORLD );
    if (world_rank == 0)
    {
        // TODO: PI result
        toss_in_circle = 0;
        for( int i = 0; world_size > i; i++ ) {
            toss_in_circle += recv[i];
        }
